...
VOID CALLBACK TimeProc(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime);	// CALLBACK 키워드가 붙은 함수 전방선언
TCHAR sTime[128];	// 그냥 시간을 담을 전역변수
...
VOID CALLBACK TimeProc(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	SYSTEMTIME st;
	GetLocalTime(&st);
	wsprintf(sTime, TEXT("지금 시간은 %2d:%2d:%2d 입니다."), st.wHour, st.wMinute, st.wSecond);
